...
2. **Every value carries a durable citation.** Each threshold / deadline / retention / definition gets the citation block adjacent to it — **source title + issuing authority (jurisdiction) + article/section + version/amendment date + stable URL + retrieved date** — or an inline `SRC-XXX` short ID into the project's source registry (`_meta/regulatory-source-registry.md`). Durable = the next reviewer verifies it in 5 years. → `${CLAUDE_PLUGIN_ROOT}/specs/workflow/07-r4-regulatory-research/03-citation-conventions.md` (block format, registry, version-pinning, URL-stability).
