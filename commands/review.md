...
Review a ZeeSpec module (or the methodology repo). Pipeline per `${CLAUDE_PLUGIN_ROOT}/specs/workflow/03-r3-deep-review.md` + `${CLAUDE_PLUGIN_ROOT}/specs/workflow/04-r1-r2-parallel-review.md`:
...
3. **Apply findings** per `${CLAUDE_PLUGIN_ROOT}/specs/workflow/05-apply-findings.md` — fold spec drift back; spawn task chips for real production bugs (`${CLAUDE_PLUGIN_ROOT}/specs/workflow/06-spawn-task-chips.md`). **Never auto-fix without review.**
