...
Author a ZeeSpec module in this project. Follow the checklist at `${CLAUDE_PLUGIN_ROOT}/specs/core/workflow/01-authoring-checklist.md`.
...
- `gravity.md` is **pointer-only** (one fact, one cell): each HW entry = `Crosses:` (the primitive cells) + `Why it's gravity:` (failure mode). Never restate the rule, a Status tag, or a `file:line` — those live in the primitive. See `${CLAUDE_PLUGIN_ROOT}/specs/core/METHODOLOGY.md` §9.
