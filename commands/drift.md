...
3. **Classify** each finding by the 4-type framework + severity per `${CLAUDE_PLUGIN_ROOT}/specs/workflow/08-code-drift-management/02-drift-categorization.md` (Type 1/2 = 🟡 P2; Type 3 = 🟠 P1→🚨 P0; Type 4 = 🚨 P0). If in doubt, escalate to the higher type.
4. **Resolve** per `${CLAUDE_PLUGIN_ROOT}/specs/workflow/08-code-drift-management/04-drift-resolution-playbook.md` — pick **exactly one** path per finding, never auto-apply:
