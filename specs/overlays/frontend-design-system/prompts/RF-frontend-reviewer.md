---
doc: overlays/frontend-design-system/prompts/RF-frontend-reviewer
type: agent-prompt
overlay: frontend-design-system
overrides: workflow/04-r1-r2-parallel-review.md (adds a frontend reviewer lane)
version: 0.1.0
status: experimental
...
> **The frontend analogue of the finance R2 reviewer** (`../../finance-accounting/prompts/R2-financial-reviewer.md`). RF reviews a *built UI* against its component + CRUD-feature specs and catches the failure modes that produced the naked-HTML / incomplete-CRUD / no-tests outcome this overlay exists to fix. Dispatch RF in the R1+R2 parallel lane (`workflow/04-r1-r2-parallel-review.md`) for any module that ships a frontend.
...
[List of P0/P1 that should become chips, per core ZeeSpec workflow/06-spawn-task-chips.md]
...
- Core ZeeSpec `workflow/04-r1-r2-parallel-review.md` — the parallel-review lane RF joins
- Core ZeeSpec `workflow/06-spawn-task-chips.md` — chip dispatch format
