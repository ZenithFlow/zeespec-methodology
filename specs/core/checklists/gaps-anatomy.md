---
doc: core/checklists/gaps-anatomy
type: checklist
version: 1.0.0
status: stable
...
>
> See also: `severity-matrix.md` (severity calibration), `status-tags.md` (lifecycle status), `core/templates/_template/gaps.md` (the template itself).
...
**Where it comes from:** `core/workflow/02-b1-verification.md` (quantitative drift check)
...
Per `core/workflow/02-b1-verification.md`. Mechanical comparison:
...
Per `core/workflow/03-r3-deep-review.md`. Agent reads ACTUAL code (not just spec), validates:
...
Per `core/workflow/04-r1-r2-parallel-review.md`. Two fresh-context reviewers:
...
Per `extended/workflow/08-code-drift-management/05-R5-drift-scanner-agent.md`. Weekly/monthly:
...
R3 finds it → R1 finds same one → R2 finds it again → 3 entries. Solution: de-dupe during apply-findings (`core/workflow/05-apply-findings.md`). Use the EARLIEST discovery as primary; cite the others as confirmations.
...
- `core/templates/_template/gaps.md` — the template itself
...
- `core/workflow/02-b1-verification.md` — B1 → drift items
- `core/workflow/03-r3-deep-review.md` — R3 → operational follow-ups
- `core/workflow/04-r1-r2-parallel-review.md` — R1/R2 → reviewer findings
- `core/workflow/05-apply-findings.md` — folding findings into gaps.md
- `core/workflow/06-spawn-task-chips.md` — gap → production fix chip
- `extended/workflow/08-code-drift-management/` — R5 drift scanner → drift items
- `extended/workflow/09-adr-lifecycle/` — ADR Proposed → RES research question
