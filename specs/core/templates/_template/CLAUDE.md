...
ADRs are summarized here (one-line per row). For full ADR text, either inline below OR in `adr/` folder per `extended/workflow/09-adr-lifecycle/01-adr-format-template.md` (Option B).
...
When this module relies on an ADR owned by another module, list it here. Don't duplicate the ADR text — cite the owner module. Per `extended/workflow/09-adr-lifecycle/03-adr-relationships.md` (inherits relationship).
