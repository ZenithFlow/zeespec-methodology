...
> Version 4.0 (post-pilot, language-agnostic, jurisdiction-neutral; three-tier core/extended/examples layout). Read once; reference forever.
...
The file set grows along the authoring DAG (`WHY → WHAT → HOW → {WHO, WHEN} → WHERE`, see `core/workflow/01-authoring-checklist.md`): Standard adds the conceptual core (why/how/gravity) on top of Lite; Full adds actors, timing, and the stack binding (`where.md`). You promote weight **incrementally** — never author files a module hasn't earned.
...
Maintenance capacity is the governor: a solo author should cap Full-weight modules at 1-2 and keep the rest Lite (`extended/workflow/10-adoption-guide/08-one-man-army.md`). The Tier-0-Lite fast-track (`extended/workflow/10-adoption-guide/07-zeespec-lite-tier-0-fasttrack.md`) is the operational how-to for authoring at Lite weight and the promotion path upward.
...
[OPTIONAL: PHASE 0 — R4 RESEARCH (1-4h per topic)]
                              → Research regulator + statute + standard for
                                ...
                                (see extended/workflow/07-r4-regulatory-research/)
       ↓
PHASE 1: AUTHOR (4-6h)     → Read existing canonical docs + production code → write 10 ZeeSpec files
                              (Author phase has 12 sub-steps — one per file + final assembly;
                               see core/workflow/01-authoring-checklist.md)
       ↓
...
> **Phase numbering note:** The pre-Tier-1 workflow has 5 phases (Author + B1 + R3 + R1+R2 + Apply). `core/workflow/06-spawn-task-chips.md` is a UTILITY used across phases (not a numbered phase). After Tier 1, two CONTINUOUS workflows take over — referenced by folder number (`extended/workflow/08`, `workflow/09`) rather than as numbered phases, since they run perpetually rather than as discrete sequential steps.
...
**The principle.** An AI reviewer is strongest on the **structural/architectural residual** — correctness, race conditions, cross-cutting coherence, dead code — because the signal is *inferable from the code itself*. It is weakest on domain/regulatory **conventions** — specific thresholds, filing deadlines, enum rules, jurisdiction definitions — because those are *not inferable from code*: they must come from an external spec, and asking a model to *recall* them invites hallucinated values (the exact failure `extended/workflow/07-r4-regulatory-research/` exists to prevent). Anthropic's context-engineering guidance makes the same split: retrieve external facts just-in-time; don't lean on model recall for them.

**The move.** Where a behavioral or regulatory rule has a *checkable value* (a threshold, a deadline, an enum allow-list, a SoD pair), encode it as a **deterministic check / executable assertion / BDD-style example** — `Given a 21M MNT cash transaction, When booked, Then a CTR auto-flags` — rather than relying on a reviewer to remember it. The **R4 output is the natural source**: its citation blocks (`extended/workflow/07-r4-regulatory-research/03-citation-conventions.md`) already pin threshold + deadline + source + date; turning each pinned value into one assertion lets a test or the **CI drift-gate** (`scripts/ci-drift-gate.sh`, alongside the `extended/workflow/08` Layer-1 scanner) enforce it *deterministically and forever* — no model recall in the loop. This is also what makes annual R4 re-validation cheap: when a statute amends a value, you change one assertion, and the gate flips. (Worked pattern: `extended/workflow/07-r4-regulatory-research/03-citation-conventions.md` § From citation to executable assertion.)

**The test-pointer rule (generalizes the above).** Not only R4 values: **every invariant whose violation would be 🚨 P0 / 🟠 P1** carries a `Test` pointer in `what.md` § 5 — a test `file:line` or an executable assertion — or a tracking gap. B1 § 6 counts the misses per module; the count is a `metrics-loop.md` column (target 0). What remains for R1/R2 is the architectural residual.
...
Most-used prefixes (see `core/checklists/invariant-numbering.md` for the **full 19-prefix catalog** including `SLA`, `G`, `BR`, `F`, `Gap`, `RES`, `NHW`):
...
Full list in `core/checklists/anti-patterns.md`. Top 5:
