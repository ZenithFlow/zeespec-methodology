---
doc: examples/overlays/frontend-design-system/principles/ui-flow-completeness
...
An operation/state that is neither built nor specified is a **completeness failure** — the reviewer reports the feature as NOT done. "It renders" is not IMPL; IMPL requires the test that proves the state, mirroring core ZeeSpec's rule that ✅ IMPL needs a real `file:line` and verification (`/specs/core/checklists/status-tags.md`).
