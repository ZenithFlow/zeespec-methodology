---
doc: examples/overlays/frontend-design-system/templates/crud-feature-spec
...
Write the flow as Gherkin so each scenario maps 1:1 to a play-function or Playwright e2e test (the citation→executable-assertion mechanism, `/specs/extended/workflow/07-r4-regulatory-research/03-citation-conventions.md`). Cover the happy path AND the error paths — error paths are where incomplete CRUDs are exposed.
...
The feature is DONE only when every row is ✅ IMPL with a citation. The reviewer fills this in; `file:line` is the proof, exactly as core ZeeSpec requires for ✅ IMPL (`/specs/core/checklists/status-tags.md`). 🟡 = built but untested; 🚧 = specified, not built. Anything below ✅ IMPL is an open item.
