---
doc: examples/overlays/finance-accounting/README
...
>
> The **methodology** (core ZeeSpec workflow + R4 regulatory research at `extended/workflow/07-r4-regulatory-research/`) is **the primary deliverable**. This overlay shows the methodology in action for one domain.
...
9. **Worked R4 research examples** (`research-examples/`) — 4 worked R4 sessions demonstrating the regulator + statute research methodology applied to Mongolia FRC, Mongolia AML law, cross-jurisdiction retention, and Mongolia NBFI lending. The R4 METHODOLOGY itself (workflow, source evaluation, citation conventions, agent prompt, cheatsheet) lives in core ZeeSpec at `extended/workflow/07-r4-regulatory-research/` (promoted to core in v2.7).

## Read order

1. **Core ZeeSpec first** — read `/specs/README.md` + `/specs/core/METHODOLOGY.md` + `/specs/core/workflow/00-START-HERE.md` (1.5h)
...
7. `/specs/extended/workflow/07-r4-regulatory-research/00-START-HERE.md` — how to research YOUR jurisdiction's specifics (regulator + statute + retention thresholds); see `research-examples/` for finance applications
...
|-------------------|------------------------|
| `core/templates/_template/CLAUDE.md` | Pre-filled module CLAUDE.md per finance module type |
| `core/templates/_template/what.md` | Pre-filled with accounting invariants (debit=credit, period close, etc.) |
| `core/templates/_template/where.md` § 5 | Still stack-specific — overlay does NOT prescribe a stack |
| `core/checklists/severity-matrix.md` | Overridden by `severity-matrix-finance.md` (FRC-tuned) |
| `core/checklists/anti-patterns.md` | Extended by `financial-anti-patterns.md` (15 more) |
| `core/workflow/04-r1-r2-parallel-review.md` R2 prompt | Replaced by `R2-financial-reviewer.md` |
| `core/templates/_template/glossary.md` | Pre-seeded with `finance-glossary.md` (finance vocabulary) |
...
cp -r path/to/zeespec-methodology/specs/examples/overlays/finance-accounting \
...
Read `docs/specs/zeespec/core/workflow/00-START-HERE.md` BEFORE any code that
...
```
examples/overlays/finance-accounting/
├── README.md                                — this file
├── principles/
