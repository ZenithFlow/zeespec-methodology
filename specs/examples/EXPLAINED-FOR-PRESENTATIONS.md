...
3. `extended/workflow/10-adoption-guide/00-START-HERE.md` уншиж — Tier шийдвэрлэх
4. 1 highly-risk модуль сонгож, champion томилох
5. 2-4 неделя дотор first Tier 1 модулийг гарга

### Хэрэв та engineer бол

1. `core/workflow/00-START-HERE.md` уншиж (AI agent entry point)
...
1. `extended/workflow/07-r4-regulatory-research/00-START-HERE.md` уншиж
2. R2 reviewer prompt (`examples/overlays/finance-accounting/prompts/R2-financial-reviewer.md`) уншиж
...
1. **ҮРГЭЛЖ** `core/workflow/00-START-HERE.md` уншиж сэтгэхүй бичих өмнө
...
| **v3.2** | **2026-06-06** | **frontend-design-system overlay (React/Next + Tailwind v4 + shadcn/ui): design tokens · component-contract spec type · ui-flow-completeness · Storybook + Playwright + axe executable tests · RF reviewer — grounded in 3 AI×design-system research passes (`examples/overlays/frontend-design-system/research-notes.md`)** |
| **v3.3** | **2026-06-06** | **Plugin steering surface: 5 skills (`zeespec-frontend` · `aware-coding` · `drift-guard` · `authoring` · `r4-citation`) that surface the right ZeeSpec guidance when a task matches their trigger, + 6 commands (`promote` · `drift` · `adr` · `gaps` · `metrics` · `check`). Skills let the methodology steer the AI without manually pointing it at the spec each time (Claude loads a skill on relevance, not a guaranteed hook).** |
| **v4.0** | **2026-06-10** | **"Practice what it preaches" minify: three-tier package layout (`core/` ≤5,000-line CI-budgeted · `extended/` · `examples/`), workflow 11+12 condensed (coverage %s → hypothesis bands, honest-numbers rule), source-cheatsheet → entry-point registry, core-budget + accretion-watch guardrails in `dogfood-drift-scan.sh`, ROADMAP-v4 (content freeze until N=2)** |
...
- **Full methodology:** `specs/core/METHODOLOGY.md`
- **AI agent entry:** `specs/core/workflow/00-START-HERE.md`
- **Adoption decision:** `specs/extended/workflow/10-adoption-guide/00-START-HERE.md`
- **Anthropic integration:** `specs/extended/workflow/11-anthropics-plugin-integration/00-START-HERE.md`
- **Pilot data:** `specs/README.md` § Validation Track Record
- **anthropics/financial-services:** https://github.com/anthropics/financial-services

---

**Бичсэн огноо:** 2026-05-18; шинэчилсэн 2026-06-10 (v4.0 — three-tier minify + guardrails)
**ZeeSpec version:** 4.0.0
**License:** MIT
