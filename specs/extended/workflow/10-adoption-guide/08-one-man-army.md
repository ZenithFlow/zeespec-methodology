---
doc: extended/workflow/10-adoption-guide/08-one-man-army
...
Agent({
  ...
  prompt: "Use prompt from extended/workflow/07-r4-regulatory-research/04-R4-agent-prompt.md.
...
- Per `extended/workflow/10-adoption-guide/07-zeespec-lite-tier-0-fasttrack.md`
...
Agent({
  ...
  prompt: "Per core/workflow/03-r3-deep-review.md, review docs/specs/zeespec/wallet/.
...
Agent({ description: "R2 compliance review", prompt: "<from workflow/04 + examples/overlays/finance-accounting/prompts/R2-financial-reviewer.md if finance>" })
```

#### Step 9: Apply findings + spawn chips (1h)
- Per `core/workflow/05-apply-findings.md`
...
- Add `extended/workflow/08-code-drift-management/03-auto-drift-detection.md` CI script
...
- **R4 agent every research session** (per `extended/workflow/07-r4-regulatory-research/`)
...
Agent({
  ...
  prompt: "Per extended/workflow/09-adr-lifecycle/05-R6-adr-curator-agent.md Mode C.
...
- Full R4 across all sources (per `extended/workflow/07-r4-regulatory-research/06-re-validation-strategy.md`)
...
- `extended/workflow/07-r4-regulatory-research/` — R4 agent (compliance research)
...
- `extended/workflow/12-agentic-role-replacement/` — 🆕 deep-dive on agentic replacement
...
→ `extended/workflow/12-agentic-role-replacement/00-START-HERE.md` for per-role agent dispatch patterns
