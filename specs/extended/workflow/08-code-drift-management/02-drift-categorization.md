---
doc: extended/workflow/08-code-drift-management/02-drift-categorization
...
- Per `core/workflow/02-b1-verification.md` recipes
...
INTENTIONAL (deliberate design change):
  → Write ADR (per extended/workflow/09-adr-lifecycle/)
...
BUG (regression / unintended behavior):
  → File spawn task chip (per core/workflow/06-spawn-task-chips.md)
...
1. **ADR documenting the architectural change** (per `extended/workflow/09-adr-lifecycle/`)
...
Cross-reference with `core/checklists/severity-matrix.md`:
...
- `extended/workflow/09-adr-lifecycle/04-drift-driven-adr-pattern.md` — Type 3+4 drift → ADR
- `core/workflow/06-spawn-task-chips.md` — Type 3+4 drift = bug → chip
- `core/checklists/severity-matrix.md` — severity calibration baseline
