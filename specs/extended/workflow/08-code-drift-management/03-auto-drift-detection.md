---
doc: extended/workflow/08-code-drift-management/03-auto-drift-detection
...
for module_dir in "$SPEC_DIR"/*/; do
  ...
  # see core/workflow/02-b1-verification.md § "Stack-specific grep patterns" for
...
echo "Categorize findings per \`extended/workflow/08-code-drift-management/02-drift-categorization.md\`." >> "$OUTPUT"
```
...
### Per-stack table — see `core/workflow/02-b1-verification.md` § "Stack-specific grep patterns"
  ...
  drift-gate:
    ...
    steps:
      ...
      - name: ZeeSpec drift gate (Type 1/2)
        ...
        # add `--json gate.json` + upload-artifact to feed _meta/metrics-loop.md from CI
        # note: the gate auto-skips the methodology tiers (core/ extended/ examples/ + README)
        # inside --spec-dir — their teaching citations are not claims about your codebase
```
...
- `core/workflow/02-b1-verification.md` — stack-specific count recipes (Type 2)
