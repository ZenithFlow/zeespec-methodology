---
doc: workflow/10-adoption-guide/01-adopting-zeespec-from-scratch
...
4. List its compliance touchpoints (per `workflow/07-r4-regulatory-research/00-START-HERE.md`)
...
Follow `workflow/01-authoring-checklist.md` exactly.
...
Run `workflow/02-b1-verification.md` — verify spec claims match production code (or planned code if greenfield).
...
Per `workflow/03-r3-deep-review.md`. Dispatch agent (or have a different engineer review). Time: 1-2 hours of agent runtime; 1 hour of human triage.

### Day 2: R1+R2 parallel reviews

Per `workflow/04-r1-r2-parallel-review.md`. Dispatch 2 agents in parallel. Time: 1-2 hours wall-clock.

### Day 3-4: Apply findings

Per `workflow/05-apply-findings.md`. Resolve P0/P1 findings. File chips for production bugs (per `06-spawn-task-chips.md`).
...
Set up CI drift detection per `workflow/08-code-drift-management/03-auto-drift-detection.md` for this module. Start in WARN mode (don't block PRs).

### Monthly

Schedule monthly drift review per `workflow/08-code-drift-management/01-drift-detection-strategies.md` (Layer 2). 2-4 hours per module.

### Quarterly

R4 re-validation if module has external-authority claims (per `workflow/07-r4-regulatory-research/06-re-validation-strategy.md`). 1-4 hours.
...
- `workflow/00-START-HERE.md` — per-module workflow (used during weeks 2-3)
- `workflow/01-authoring-checklist.md` — first-module authoring detail
