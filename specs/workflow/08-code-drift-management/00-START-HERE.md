---
doc: workflow/08-code-drift-management/00-START-HERE
type: workflow-entry
...
**Detection:** B1 verification quantitative grep (per `workflow/02-b1-verification.md`).
...
- If intentional design change: write ADR (per `09-adr-lifecycle/`) documenting the change; update spec to reflect new reality; bump status tags
...
**Resolution:**
- Write architectural ADR (per `09-adr-lifecycle/`) for the change
...
- **Annually:** full re-validation per `workflow/02-b1-verification.md`
...
This is the **drift-driven ADR pattern** (see `09-adr-lifecycle/04-drift-driven-adr-pattern.md`). Every architectural drift should trigger an ADR. Without it, the spec keeps lying about reality.
...
- `workflow/02-b1-verification.md` — initial quantitative drift detection
- `workflow/03-r3-deep-review.md` — line-by-line drift in deep review
- `workflow/05-apply-findings.md` — folding drift findings back
- `workflow/06-spawn-task-chips.md` — when drift = production bug
- `workflow/09-adr-lifecycle/` — drift that reveals undocumented design decisions
- `checklists/severity-matrix.md` — severity calibration for drift findings
- `checklists/anti-patterns.md` — drift-related anti-patterns
