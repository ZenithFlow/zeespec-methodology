...
# drift types from extended/workflow/08-code-drift-management/02-drift-categorization.md:
...
# they need the R5 agent (extended/workflow/08 .../05-R5-drift-scanner-agent.md), not a gate.
#
...
#
# Usage:
#   scripts/ci-drift-gate.sh [--spec-dir DIR] [--code-dir DIR] [--json FILE]
#   ZEESPEC_DRIFT_MODE=fail scripts/ci-drift-gate.sh
#   ZEESPEC_GATE_JSON=gate.json scripts/ci-drift-gate.sh   # same as --json
# Defaults: --spec-dir docs/specs/zeespec  --code-dir .
#   exit 0 = pass / WARN mode · 1 = drift (FAIL mode) · 2 = setup error
# --json writes a machine-readable summary so the metrics loop (core/templates/
# _meta/metrics-loop.md) can be filled from CI artifacts instead of by hand.
...
checked=0; broken=0; ann=0; bad=0
emit_json() { # $1 = result (pass|warn|fail)
  [ -n "$JSON_OUT" ] || return 0
  printf '{"schema":"zeespec-gate/1","mode":"%s","spec_dir":"%s","citations_checked":%d,"citations_broken":%d,"counts_annotated":%d,"counts_drifted":%d,"drift_total":%d,"result":"%s"}\n' \
    "$MODE" "$SPEC_DIR" "$checked" "$broken" "$ann" "$bad" "$drift" "$1" > "$JSON_OUT"
...
# The methodology's own tiers (core/ extended/ examples/ + the router README) ship
# INSIDE the adopter's spec dir and are full of illustrative teaching citations —
# they are not claims about THIS codebase, so the gate only scans module specs.
spec_files() {
  find "$SPEC_DIR" -type f -name '*.md' \
    ! -path "$SPEC_DIR/core/*" ! -path "$SPEC_DIR/extended/*" \
    ! -path "$SPEC_DIR/examples/*" ! -path "$SPEC_DIR/README.md" 2>/dev/null
}
...
if [ "$drift" -eq 0 ]; then
  ...
  echo "are semantic; dispatch the R5 agent — see extended/workflow/08-code-drift-management.)"
  emit_json pass
  exit 0
fi
if [ "$MODE" = "fail" ]; then
